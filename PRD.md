...
| Label circularity (CNN just re-learns the threshold rule) | Explicitly frame contribution as spatial/textural generalization beyond pixel-wise thresholding; address directly in report/viva | **CONFIRMED IN SYNTHETIC DEMO.** Circularity check `flag_suspicious_metrics` triggered for Multimodal models (F1=0.0, Acc=1.0). Grad-CAM overlays in `reports/gradcam/` generated for visual analysis. Requires real data to assess definitively. |
